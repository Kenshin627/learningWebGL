...
void main(){
    ...
    vec3 emmisiveTexture = vec3(texture(material.esampler, texcoord));
    ...
    //specular
    vec3 specular = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess) * 4.0 * specularTexture;

    outColor = vec4((emmisiveTexture + specular) * light.intensity, 1.0);
}